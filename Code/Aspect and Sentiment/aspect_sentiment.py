...
# ]
#
#     top_20_score = [ 0.060378663691711527,
#  0.060575877753149591,
#  0.062514952154405459,
#  0.063233798974291031,
#  0.064901816692092981,
#  0.066948822742093198,
#  0.067917947124193273,
#  0.068805357787801849,
#  0.06968711290029056,
#  0.069735779340854648,
#  0.07259727246652041,
#  0.075290890616145106,
#  0.080266664783137501,
#  0.089042311062329502,
#  0.08976854854095799,
#  0.089804303204613856,
#  0.091266509637135229,
#  0.092554906745723989,
#  0.094314098884660746,
#  0.097496191541690688,
#  0.098197433019735672,
#  0.098493344175212244,
#  0.10086549347064051,
#  0.10958479610705732,
#  0.16142716245472874,
#  0.19665584771516362
   ...
   # dataset = "reviews_Books_5.json.gz"
    for i, review in enumerate(parse(dataset)):
        ...
        if (prev_product) == '':        #initialize the value
            score_array = np.array([similarity.aspect_included(num_review, product_id, this_review,threshold, "minmax")])  # 0.481 for avg



        if (product_id == prev_product):
            aspect_score = np.array([similarity.aspect_included(num_review, product_id, this_review, threshold, "minmax")])   # 0.481 for avg
        ...
        if prev_product == '': #initialte the first one
            ...
            review_person.append(review['reviewText'])
            ################analyze each review which aspects are inluded#####################
            score_array = np.array([similarity.aspect_included(num_review, prev_product, review['reviewText'], threshold, "minmax")])  # 0.481 for avg
            ################analyze each review which aspects are inluded#####################
        # else:
        #     pass
